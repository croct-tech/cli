...
export function buildPropertyExpression(attribute: AttributeType): t.Expression {
...
export function createPropsObject(attributes: Record<string, AttributeType>): t.ObjectExpression {
    return t.objectExpression(
        Object.entries(attributes).map(
            ([key, value]) => t.objectProperty(
                t.identifier(key),
                buildPropertyExpression(value),
            ),
        ),
    );
}
